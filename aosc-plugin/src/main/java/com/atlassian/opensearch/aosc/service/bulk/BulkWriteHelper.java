...
public final class BulkWriteHelper {
    ...
    public static CompletableFuture<BulkOutcome> submitBulkAsyncWithOutcome(
        ...
        return attempt(clientHelper, request, logger, 0, threadPool);
    }
    ...
    private static CompletableFuture<BulkOutcome> attempt(
        ...
        return clientHelper.executeBulkAsync(request).<CompletableFuture<BulkOutcome>>handle((response, ex) -> {
            ...
            return scheduleRetry(clientHelper, request, logger, attemptCount, threadPool, delayMs);
        }).thenCompose(f -> f);
    }
    ...
    private static CompletableFuture<BulkOutcome> scheduleRetry(
        ...
        AsyncUtils.scheduleDelayed(
            threadPool,
            delayMs,
            () -> attempt(clientHelper, request, logger, nextAttemptNum, threadPool).whenComplete((o, e) -> {
