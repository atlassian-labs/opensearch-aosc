...
import com.atlassian.opensearch.aosc.compat.OsCompat;
...
public class CutoverService {
    ...
    public CutoverService(AoscLogger logger, AsyncClientHelper clientHelper, IndexOperationUtils indexOperationUtils, String migrationId) {
    ...
    CompletableFuture<Void> refreshIndex(String index) {
        return clientHelper.executeRefreshAsync(new RefreshRequest(index)).thenApply(response -> {
    ...
    CompletableFuture<Long> countDocs(String index, QueryBuilder validationQuery) {
        ...
        return clientHelper.executeSearchAsync(searchRequest)
            .thenApply(response -> OsCompat.totalHitCount(response.getHits().getTotalHits()));
    }
