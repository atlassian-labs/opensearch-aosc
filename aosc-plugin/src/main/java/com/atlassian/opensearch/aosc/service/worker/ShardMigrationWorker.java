...
public class ShardMigrationWorker implements Closeable {
    ...
    private final CoordinatorUpdateChannel coordinatorUpdates;
    ...
    public ShardMigrationWorker(
        ...
        this.coordinatorUpdates = new CoordinatorUpdateChannel(
            logger,
            client,
            threadPool,
            migrationId,
            shardId,
            () -> buildProgress(sm.currentState()),
    ...
    private AwaitableStateMachine<ShardPhase> buildStateMachine() {
        ...
        builder.writeBarrier((from, to) -> {
            ...
            return coordinatorUpdates.reportTransition(to);
        });
    ...
    private void onProgressTick() {
        if (ShardPhase.TERMINALS.contains(sm.currentState())) return;
        // Heartbeat. The channel reads the current phase at send time and keeps a single send in
        // flight, so a tick can never reorder past the (blocking) write barrier.
        coordinatorUpdates.heartbeat();
    }
    ...
    /** Build a point-in-time snapshot of this shard's progress for the given phase. */
    private ShardProgressDocument buildProgress(ShardPhase phase) {
