...
public class TargetIndexEmptyValidatorTests extends OpenSearchTestCase {
    ...
    private static ValidationContext ctx(AsyncClientHelper clientHelper) {
    ...
    private static AsyncClientHelper mockClientWithDocCount(long docCount) {
        var client = MockClientFactory.mockClient();
        var adminClient = MockClientFactory.mockAdminClient();
        var indicesAdminClient = MockClientFactory.mockIndicesAdminClient();
    ...
    public void testEmptyTargetPasses() throws Exception {
        var client = mockClientWithDocCount(0);
    ...
    public void testNullDocsTreatedAsEmpty() throws Exception {
        var client = mockClientWithDocCount(-1); // -1 sentinel => total.getDocs() returns null
        CompletableFuture<Void> f = new TargetIndexEmptyValidator().validate(ctx(client));
        f.get(5, TimeUnit.SECONDS);
    }

    public void testNonEmptyTargetRejects() throws Exception {
        var client = mockClientWithDocCount(42);
        CompletableFuture<Void> f = new TargetIndexEmptyValidator().validate(ctx(client));
    ...
    public void testClientFailurePropagates() throws Exception {
        ...
        CompletableFuture<Void> f = new TargetIndexEmptyValidator().validate(ctx(AsyncClientHelper.wrap(client)));
        ExecutionException ee = expectThrows(ExecutionException.class, () -> f.get(5, TimeUnit.SECONDS));
