...
public class ValidationQueryValidatorTests extends OpenSearchTestCase {
    ...
    private static ValidationContext ctx(MigrationRequestOptions options, AsyncClientHelper clientHelper) {
        MigrationRequest req = new MigrationRequest().setSourceIndex("src").setTargetIndex("tgt").setOptions(options);
        return ValidationContext.of(req, ClusterState.EMPTY_STATE, null, null, new TransformFactory(null), clusterSettings(), clientHelper);
    }

    @SuppressWarnings("unchecked")
    private static AsyncClientHelper mockClientSuccess() {
    ...
    @SuppressWarnings("unchecked")
    private static AsyncClientHelper mockClientFailure(String errorMsg) {
        var client = MockClientFactory.mockClient();
        doAnswer(inv -> {
            ActionListener<SearchResponse> listener = (ActionListener<SearchResponse>) inv.getArguments()[1];
    ...
    public void testNoOpWhenNoOptions() throws Exception {
        var client = MockClientFactory.mockClient();
        validator.validate(ctx(null, AsyncClientHelper.wrap(client))).get(5, TimeUnit.SECONDS);
        verify(client, never()).search(any(), any());
    }

    public void testNoOpWhenNullValidationQuery() throws Exception {
        var client = MockClientFactory.mockClient();
        validator.validate(ctx(new MigrationRequestOptions(), AsyncClientHelper.wrap(client))).get(5, TimeUnit.SECONDS);
        verify(client, never()).search(any(), any());
    }
    ...
    public void testNoOpWhenEmptyValidationQuery() throws Exception {
        ...
        validator.validate(ctx(opts, AsyncClientHelper.wrap(client))).get(5, TimeUnit.SECONDS);
        verify(client, never()).search(any(), any());
    }
    ...
    public void testDryRunFailurePropagates() throws Exception {
        var client = mockClientFailure("field [status] not found");
        MigrationRequestOptions opts = new MigrationRequestOptions();
        opts.setValidationQuery(Map.of("term", Map.of("status", "active")));
